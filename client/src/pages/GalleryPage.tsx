...
const GalleryPage = () => {
  ...
  useEffect(() => {
    const fetchGallery = async () => {
      try {
        const response = await fetch('/admin/api/data/gallery');
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background pt-24 flex justify-center items-center">
        <div className="text-xl text-white">Loading Gallery...</div>
      </div>
    );
  }
      ...
      <div className="px-4 py-8">
        ...
        {images.length > 0 ? (
            ...
            {images.map((item, index) => (
                <div
                key={item.id || index} // Fallback to index if DB id is missing
        ...
        {selectedImageIndex !== null && (
          ...
          >
            ...
            >
              ...
              {/* Dot Indicators */}
              <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex space-x-2 z-10 overflow-x-auto max-w-[80vw] p-2 no-scrollbar">
                {images.map((_, index) => (
                  <button
                    key={index}
